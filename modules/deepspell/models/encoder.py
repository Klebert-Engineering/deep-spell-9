...
class DSVariationalLstmAutoEncoder(modelbase.DSModelBase):
    ...
    def _encoder(self):
        ...
        with tf.name_scope("encoder"):
            ...
            tf_encoder_backward_cell = tf.compat.v1.nn.rnn_cell.MultiRNNCell([
                tf.compat.v1.nn.rnn_cell.BasicLSTMCell(hidden_state_size) for hidden_state_size in
                self.encoder_bw_state_size_per_layer])
            tf_encoder_forward_cell = tf.compat.v1.nn.rnn_cell.MultiRNNCell([
                tf.compat.v1.nn.rnn_cell.BasicLSTMCell(hidden_state_size) for hidden_state_size in
                self.encoder_fw_state_size_per_layer])
            tf_encoder_combine_cell = tf.compat.v1.nn.rnn_cell.MultiRNNCell([
                tf.compat.v1.nn.rnn_cell.BasicLSTMCell(hidden_state_size) for hidden_state_size in
